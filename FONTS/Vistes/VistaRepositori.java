...
public class VistaRepositori {
    private ControladorPresentacio controladorPresentacio;
    private JPanel panel;
    private JScrollPane scrollPane;
    private celaComponent[][] celes;
    private JLabel labelRepositori;
    private MyButton esborrar, editar;
    ...
    public VistaRepositori(ControladorPresentacio controladorPresentacio) {
    ...
    private void crearBoto(){
        int widthButton = 150;
        int heightButton = 70;
        esborrar = new MyButton(widthButton, heightButton, "Esborrar");
        esborrar.setBounds(2*width/3-widthButton, height-heightButton*2, widthButton, heightButton);
        editar = new MyButton(widthButton, heightButton, "Editar");
        editar.setBounds(2*width/3, height-heightButton*2, widthButton, heightButton);
    }
    private void inicialitzarComponents(){
        ...
        panel.add(labelRepositori);
    }
    private void crearLabel(){
        labelRepositori = new JLabel("Repositori");
        int widthLabel = 400;
        int heightLabel = 60;
        Font font = new Font("Arial", Font.BOLD, 40);
        labelRepositori.setFont(font);
        labelRepositori.setForeground(new Color(230,230,230));
        labelRepositori.setBackground(new Color(39,54,90));
        labelRepositori.setHorizontalAlignment(SwingConstants.CENTER);
        labelRepositori.setMaximumSize(new Dimension(widthLabel, heightLabel));
        labelRepositori.setMinimumSize(new Dimension(widthLabel, heightLabel));
        labelRepositori.setPreferredSize(new Dimension(widthLabel, heightLabel));
        labelRepositori.setBounds(width/2-widthLabel/2, heightLabel/2, widthLabel, heightLabel);
    }
    ...
    private void asignar_listenersComponentes() {
        ...
        esborrar.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseReleased(MouseEvent e) {
                super.mouseReleased(e);
                int index = list.getSelectedIndex();
                if (index != -1){
                    controladorPresentacio.borrarRepositori(repositoris[index]);
                    String[] tmp = new String[repositoris.length - 1];
                    int j = 0;
                    for (int i = 0; i < repositoris.length; i++) {
                        if (i != index) {
                            tmp[j++] = repositoris[i];
                    ...
                    if (index == repositoris.length) --index;
                    list.setSelectedIndex(index);
                    scrollPane.revalidate();
                    scrollPane.repaint();
                    boardKakuro.removeAll();
                    if (index >= 0)crearKakuro(controladorPresentacio.getRepositori(repositoris[index]));
                    boardKakuro.revalidate();
                    boardKakuro.repaint();
                }
                if (repositoris.length == 0){
                    boardKakuro.setVisible(false);
                }
        ...
        editar.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                super.mouseReleased(e);
                int index = list.getSelectedIndex();
                if (index != -1)controladorPresentacio.canviRepositoriEditar(repositoris[index]);
            }
        });
    ...
    public void actualitzarRepositori() {
